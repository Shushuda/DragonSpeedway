...
### Libraries (in `Libs/`)
- **AceDB-3.0** - Profile-based saved variables with per-character support
...
- `PLAYER_CAN_GLIDE_CHANGED` - Detects dynamic flight mount/dismount
- `PLAYER_MOUNT_DISPLAY_CHANGED` - Fallback dismount detection
- `UNIT_SPELLCAST_START/FAILED/INTERRUPTED` - Dracthyr Soar detection for camera exclusion
